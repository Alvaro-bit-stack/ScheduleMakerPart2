...
class App {
  ...
  setOnItemClick(callback: (item: { image: string; text: string }, index: number) => void) {
  ...
  onClick(e: MouseEvent) {
    ...
    const clickedItem = this.findClickedItem(worldPos);
    if (clickedItem && this.onItemClick) {
      this.onItemClick(clickedItem.item, clickedItem.index);
    }
  }
  ...
  findClickedItem(worldPos: { x: number; y: number }) {
    for (let i = 0; i < this.medias.length; i++) {
      const media = this.medias[i];
      const planePos = media.plane.position;
      const planeScale = media.plane.scale;
      
      // Check if click is within the bounds of this media item
      const halfWidth = planeScale.x / 2;
      const halfHeight = planeScale.y / 2;
      
      const isWithinBounds = Math.abs(worldPos.x - planePos.x) <= halfWidth && 
                            Math.abs(worldPos.y - planePos.y) <= halfHeight;
      
      if (isWithinBounds) {
        return {
          item: this.mediasImages[i],
          index: i
        };
      }
    }
...
export default function CircularGallery({
  ...
  // Set the callback function when onItemClick changes
  useEffect(() => {
    if (appRef.current && onItemClick) {
      appRef.current.setOnItemClick(onItemClick);
    }
  }, [onItemClick]);
